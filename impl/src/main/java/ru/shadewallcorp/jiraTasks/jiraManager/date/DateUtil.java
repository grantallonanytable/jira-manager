...
import static ru.shadewallcorp.jiraTasks.jiraManager.date.DateStatus.*;
...
public class DateUtil {
    // TODO: congigMap, application.conf
    private static final String PATH_TO_NOT_WORK_DAYS = "src/main/java/ru/shadewallcorp/jiraTasks/jiraManager/date/data/notWorkDays.txt";
    private static final String PATH_TO_WORK_HOLIDAYS = "src/main/java/ru/shadewallcorp/jiraTasks/jiraManager/date/data/workHolidays.txt";
    ...
    static {
        ...
        CalendarConfig config = ConfigBeanFactory.create(ConfigFactory.load().getConfig("calendar"), CalendarConfig.class);
        notWorkDaysList = config.getNotWorkDaysList().stream()
                .map(s -> { try { return DateUtil.DATE_FORMAT.parse(s); } catch (ParseException e) {return null;}})
                .collect(Collectors.toList());
        workHolidaysList = config.getWorkHolidaysList().stream()
                .map(s -> { try { return DateUtil.DATE_FORMAT.parse(s); } catch (ParseException e) {return null;}})
                .collect(Collectors.toList());
    ...
    //TODO: убрать устаревший форматтер
     ...
     * @return объект Time
     */
    public static Time time(long days, long hours, long minutes) {
        return new Time (days, hours, minutes);
    }

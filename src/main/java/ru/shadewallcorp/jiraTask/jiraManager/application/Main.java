...
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.GENERATE_FULL_REPORT;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.GENERATE_REPORT_BY_ISSUE;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.GENERATE_SHORT_REPORT;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.SET_SCRUM;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.UPDATE_DEADLINE;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.UPDATE_LABELS;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.getAction;
import static ru.shadewallcorp.jiraTask.jiraManager.util.Action.getAllActions;
...
public class Main {
    ...
    public static void main1(String[] args) throws Exception {
        ...
        if(getAction(action) == GENERATE_REPORT_BY_ISSUE)
            ReportGenerator.saveReport(userName, password, issueId, null, null, false, reportPath);
        if(getAction(action) == GENERATE_FULL_REPORT)
            ReportGenerator.saveReport(userName, password, null, date(dateFrom), date(dateTo), true, reportPath);
        if(getAction(action) == GENERATE_SHORT_REPORT)
            ReportGenerator.saveReport(userName, password, null, date(dateFrom), date(dateTo), false, reportPath);

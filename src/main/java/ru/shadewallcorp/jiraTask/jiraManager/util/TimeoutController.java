...
public final class TimeoutController {
     ...
     * @throws TaskTimeoutException if the timeout passes and the thread does not return.
     */
    public static void execute(Thread task, long timeout) throws TaskTimeoutException {
        task.start();
        try {
            task.join(timeout);
        ...
        if (task.isAlive()) {
            task.interrupt();
            throw new TaskTimeoutException("Timeout while running job.");
        }
    }
     ...
     * @throws TaskTimeoutException if the timeout passes and the thread does not return.
     */
    public static void execute(Runnable task, long timeout) throws TaskTimeoutException {
